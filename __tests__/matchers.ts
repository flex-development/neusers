import type { NullishNumber } from '@flex-development/dreepo'
 ...
 *
 * @param {PlainObject} received - `supertest.Response` headers object
...
export const toHaveContentType = (
  ...
  const hasContentType = () => {
    ...
    return type ? (M.toMatch(type, expected) as PlainObject).pass : false
  }
 ...
 *
 * @param {any} received - `supertest.Response` object
...
export const toHaveStatus = (
  ...
  const hasStatus = () => {
    ...
    return (M.toBe(response.status, expected) as PlainObject).pass
  }
